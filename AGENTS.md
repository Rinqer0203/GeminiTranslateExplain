...
QuickExplain は、Windows 向けの WPF デスクトップアプリです。クリップボード、グローバルショートカット、スクリーンショットをトリガーにして、Google / OpenAI へ問い合わせ、翻訳・説明・任意プロンプトの結果を表示します。
...
- `QuickExplain/Services/ApiClients`: Google / OpenAI のストリーミングAPIクライアント。
- `README.md`: ユーザー向け概要。
...
- AIモデルは `AiModel(Name, Type)` で表現します。Google のモデル名は API URL 側で `models/{modelName}` として使うため、設定には `models/` なしで保存します。
